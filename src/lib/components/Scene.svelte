<script>
  ...
  import { Color, MeshBasicMaterial, PMREMGenerator, PlaneGeometry, Raycaster, SphereGeometry, Vector2, Vector3 } from 'three';
  ...
  const setupEffectComposer = () => {
    ...
    const bloomPass = new UnrealBloomPass(new Vector2(innerWidth, indexedDB), 0.275, 1, 0);
  ...
  useRender(({scene}) => {
    ...
    spaceShipRef.traverse((child) => {
      if (child?.material?.envMapIntensity) {
        ...
        child.material.envMapIntensity = 35;
  ...
  onMount(() => {
    ...
    function onPointerMove(event) {
      ...
      if (intersectionPoint) {
        // this prevent the spring motion to be different while the pointer
        // span the x axis
